...
$OUT = array();
$workdir = "http://www.southnegocios.com/sistema".$uploaddir;
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if($JSON["method"] == "zip" || $JSON["method"] == "unzip")
    {
        ...
        $OUT["error"] = array(
            "code"=>"100",
            "message"=>"Este método não é suportado!",
            "data"=>""
        );
        $OUT["id"] = $id;
    }
    ...
    if($method == "listFiles")
    {
        ...
        else
        {
            ...
            $path = iconv('utf-8', $charset, $path);
            $stat = stat("http://www.southnegocios.com/sistema/arquivos".$path);
            $modify = date($stat[9]);
    ...
    if($method == "moveTo")  // ��������������, �������
    {
        ...
        else
        {
            ...
            if(!strstr($from,"..") && !strstr($to,"..") && !in_array($type,$forbidden))
            {
               ...
               $OUT["error"] = array("code"=>"100","message"=>"Este tipo de arquivo é proibido! Renomear cancelado!","data"=>"");
        }
        $OUT = refreshdirs();
        $OUT["id"] = $id;
    }
