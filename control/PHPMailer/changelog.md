...
* Improve example img, switch to PNG
...
* added ability to define path (mainly for embedded img)
  function `MsgHTML($message,$basedir='')` ... where:
  `$basedir` is the fully qualified path
* fixed `MsgHTML()` function:
  - Embedded Images where img are specified by `<protocol>://` will not be altered or embedded
