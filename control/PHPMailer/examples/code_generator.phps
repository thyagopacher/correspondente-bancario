...
try {
    if (isset($_POST["submit"]) && $_POST['submit'] == "Submit") {
        ...
        $mail->msgHTML($body, dirname(__FILE__), true); //Create message bodies and embed img

        $example_code .= "\n\$mail->WordWrap = 80;";
        $example_code .= "\n\$mail->msgHTML(\$body, dirname(__FILE__), true); //Create message bodies and embed img";

        $mail->addAttachment('img/phpmailer_mini.png', 'phpmailer_mini.png'); // optional name
        $mail->addAttachment('img/phpmailer.png', 'phpmailer.png'); // optional name
